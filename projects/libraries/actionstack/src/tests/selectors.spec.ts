...
describe('selector', () => {
  ...
  it('should return undefined when selector returns nullish', () => {
    const selectNil = selector((_state: TestState) => null as any);
    expect(selectNil(mockState)).toBeUndefined();
  });

  it('should return undefined when selector throws', () => {
    const bad = selector((_state: TestState) => {
      throw new Error('boom');
    });
    expect(bad(mockState)).toBeUndefined();
  });
  ...
  it('should return undefined when any input selector returns nullish', () => {
    const projector = jasmine.createSpy('projector').and.callFake(() => 'x');

    const select = selector(
      (_state: TestState) => null as any,
      (_state: TestState) => 1,
      projector as any
    );

    expect(select(mockState)).toBeUndefined();
...
describe('selectorAsync', () => {
  ...
  it('should return undefined when async selector resolves to nullish', async () => {
    const selectNil = selectorAsync(async (_state: TestState) => null as any);
    expect(await selectNil(mockState)).toBeUndefined();
  });

  it('should return undefined when single selector throws', async () => {
    const bad = selectorAsync((_state: TestState) => {
      throw new Error('boom');
    });
    expect(await bad(mockState)).toBeUndefined();
  });
  ...
  it('should return undefined when any input selector returns nullish (async projector not called)', async () => {
    const projector = jasmine
      .createSpy('projector')
      .and.callFake(async () => 'x');

    const select = selectorAsync(
      (_state: TestState) => undefined as any,
      (_state: TestState) => 1,
      projector as any
    );

    expect(await select(mockState)).toBeUndefined();
    expect(projector).not.toHaveBeenCalled();
  });
});
